...
export class GamePageComponent implements OnInit{
    ...
    constructor(private route: ActivatedRoute, private gameService: GameService, private reviewService: ReviewService,private userService: UsersService, private userListService: UserlistService){
    ...
    getBook(id:number){
      this.gameService.getGameById(id).subscribe((book) => {this.game = book;
    ...
    saveOption(){
      ...
      for (let i = 0; i < this.lists.length; i++) {
        if (this.lists[i].name != this.selectedOption) {
          this.userListService.deletetGameFromList(this.lists[i].name, this.g).subscribe((userlist) => {
      ...
      if (this.selectedOption != 'AddBook' && this.selectedOption != 'Delete') {
        this.userListService.postGameToList(this.selectedOption, this.g).subscribe((userlist) => {
    ...
    getListOfBook(){
      this.userListService.getListOfGame(this.id).subscribe((list) => {this.lists2 = list;
